...
import { useModelRegistry, type AIModel } from "../components/ModelRegistry";
...
export default function APIKeySettings() {
  ...
  const recId = useMemo(() => {
    let b: string | null = null,
      bs = -1;
    for (const [id, a] of Object.entries(agg) as [string, { sr: number; al: number; mn: string; t: number; s: number }][]) {
      if (!a.sr) continue;
  ...
  useEffect(() => {
    for (const [mid, r] of Object.entries(testRes) as [string, ConnectivityTestResult][]) {
  ...
  const doTest = async (model: AIModel) => {
    if (testId) return;
    setTestId(model.id);
    let tp: import("../services/LLMService").ProviderConfig | undefined;
  ...
  const drip = (
    ...
    if (list.length === 0) {
      if (mRef.current) { setOlScan(false); setOlConn(conn); }
    } else {
      nx();
    }
  };
