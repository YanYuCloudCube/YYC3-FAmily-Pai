...
export default function DiffPreviewModal({
  ...
  const selectedCount = useMemo(
    () => Object.values(decisions as Record<string, FileDecision>).filter((d) => d.accepted).length,
    [decisions],
  );
  ...
  const selectAll = useCallback(() => {
    setDecisions((prev) => {
      ...
      for (const [k, v] of Object.entries(prev) as [string, FileDecision][]) {
  ...
  const addedLines = useMemo(() => {
    let count = 0;
    for (const [fp, d] of Object.entries(decisions) as [string, FileDecision][]) {
      if (d.accepted) {
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {plan.blocks.map((block) => {
            ...
            return (
              <div
                key={block.filepath}
                className={`border-b border-[var(--ide-border-faint)] last:border-b-0 ${!decision.accepted ? "opacity-50" : ""
                  }`}
              >
                {/* File Header */}
                <div className="flex items-center gap-2 px-4 py-2 bg-[var(--ide-bg-surface)] hover:bg-white/[0.02] transition-colors">
                  ...
                  <button
                    ...
                    className={`w-4.5 h-4.5 rounded border flex items-center justify-center transition-all flex-shrink-0 ${decision.accepted
                  ...
                  {/* Badge */}
                  <span
                    className={`text-[0.58rem] px-1.5 py-0.5 rounded-full flex-shrink-0 ${block.isNew
