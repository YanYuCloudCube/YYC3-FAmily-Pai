...
export async function detectOllama(): Promise<{
  ...
  try {
    log.info('Attempting to detect Ollama at http://localhost:11434/api/tags');
    ...
    log.info('Ollama response status:', res.status, res.ok);

    if (!res.ok) {
      log.error('Ollama response not OK:', res.status, res.statusText);
      return { available: false, models: [] };
    }

    const data = await res.json();
    log.info('Ollama detected models:', data.models?.length || 0);
...
export async function chatCompletionWithRetry(
  ...
  const circuitKey = `llm:${provider.id}:${modelId}`

  if (!globalCircuitBreaker.canExecute(circuitKey)) {
    throw new Error(
      `[${provider.name}] 熔断器已开启 — 服务暂时不可用，请稍后重试或切换 Provider`,
    )
  }
  ...
  for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
    try {
      const result = await chatCompletion(provider, modelId, messages, options)
      ...
      if (!isRetryableError(err, status, retryConfig)) {
        globalCircuitBreaker.recordFailure(circuitKey)
        throw lastError
      }
...
export async function chatCompletionStreamWithRetry(
  ...
  for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
    ...
    const wrappedCallbacks: StreamCallbacks = {
      ...
      onError: (error) => {
        failed = true
        lastError = error
        const statusMatch = error.match(/API 错误 (\d+)/)
        if (statusMatch) {
    ...
    if (!isRetryableError(null, httpStatus, retryConfig)) {
      globalCircuitBreaker.recordFailure(circuitKey)
  ...
  globalCircuitBreaker.recordFailure(circuitKey)
  callbacks.onError(lastError || "所有重试均失败")
}
