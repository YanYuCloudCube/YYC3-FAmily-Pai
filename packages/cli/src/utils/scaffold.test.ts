...
describe('defaultScaffold', () => {
  ...
  it('should clean up the temp directory after extraction', async () => {
    ...
    expect(vi.mocked(fs.remove)).toHaveBeenCalledWith(expect.stringContaining('shadcn-template-'));
  });
  ...
  it('should use local templates when SHADCN_TEMPLATE_DIR is set', async () => {
    ...
    expect(
      execaCalls.some((call) => call[0] === 'git' && (call[1] as string[]).includes('clone'))
    ).toBe(false);
  ...
  it('should remove pnpm-lock.yaml for non-pnpm package managers', async () => {
    ...
    expect(vi.mocked(fs.remove)).toHaveBeenCalledWith(path.join('/test/my-app', 'pnpm-lock.yaml'));
  });
  ...
  it('should not remove pnpm-lock.yaml when using pnpm', async () => {
    ...
    expect(removeCalls.some((call) => call[0].toString().includes('pnpm-lock.yaml'))).toBe(false);
  });
  ...
  it('should pass --no-frozen-lockfile for pnpm', async () => {
    ...
    expect(vi.mocked(execa)).toHaveBeenCalledWith('pnpm', ['install', '--no-frozen-lockfile'], {
      cwd: '/test/my-app',
    });
  });

  it('should strip packageManager field from package.json for non-pnpm non-monorepo', async () => {
    vi.mocked(fs.existsSync).mockImplementation((p: any) => p.toString().includes('package.json'));
  ...
  it('should convert pnpm-workspace.yaml to workspaces field for non-pnpm monorepo', async () => {
    ...
    vi.mocked(fs.readFile).mockImplementation(((filePath: string) => {
      ...
      return Promise.resolve(JSON.stringify({ name: 'my-mono', packageManager: 'pnpm@9.0.0' }));
    }) as any);
  ...
  it('should rewrite workspace: protocol refs to * for npm monorepo', async () => {
    ...
    const nestedWrite = writeCalls.find(
      (call) => (call[0] as string).includes('apps') && (call[0] as string).includes('package.json')
    );
  ...
  it('should write project name to package.json', async () => {
    vi.mocked(fs.existsSync).mockImplementation((p: any) => p.toString().includes('package.json'));
    vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({ name: 'template-name' }) as any);
